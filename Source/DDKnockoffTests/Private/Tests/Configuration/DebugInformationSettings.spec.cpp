...
BEGIN_DEFINE_SPEC(FDebugInformationSettingsAssetValidationSpec,
                  "DDKnockoff.00_Configuration.DebugInformationSettings",
...
END_DEFINE_SPEC(FDebugInformationSettingsAssetValidationSpec)

void FDebugInformationSettingsAssetValidationSpec::Define() {
    BeforeEach([this] {
        // SPEC_BOILERPLATE_BEGIN
    ...
    It("should load and validate the default settings asset",
       [this] {
           ...
           if (GameSettings && !GameSettings->DebugInformationSettingsAsset.IsNull()) {
                UDebugInformationSettings* TestSettings = Cast<UDebugInformationSettings>(
                   GameSettings->DebugInformationSettingsAsset.TryLoad());
